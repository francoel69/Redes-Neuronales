...
int neuron_d(const matrix xi, const matrix m, int i, int p);

int main(void){
  matrix m, xi, s, saux;
  int i= 0, j= 0, med= 0, p= 0, k= 0, m_medio= 0, mu= 0;
...
//   w= create_matrix(N, N);
  ...
  for(p=4;p<=PMAX;p+=4){
	...
	for(i=0;i<N;i++){
	  s= set(i, 0, neuron_d(xi,m,i,p), s);
	  for(mu=0;mu<p;mu++){
	    m= set(mu,0,get(i,mu,xi)*(get(i,0,s)-get(i,0,saux)),m);
	  }
	}
      }
      med= 0;
      for(j=0;j<N;j++){
	med += get(j,0,s)*get(j,0,xi);
      }
      m_medio += abs(med);
    }
